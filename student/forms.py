...
class StudentForm(forms.ModelForm):
    ...
    def clean(self):
        cleaned_data = self.cleaned_data # se generareaza un dictionar cu datele completate in formular
        # O unicitate pt adresa de email
        get_email = cleaned_data.get('email')     # cleaned_date['email']


        # unicitate pe adresa mail
        check_email = Student.objects.filter(email=get_email)
        if check_email:
            msg = 'Exista deja aceasta adresa de email'
            self._errors['email'] = self.error_class([msg]) # afisez pe fieldul email eraore din variabila msg
